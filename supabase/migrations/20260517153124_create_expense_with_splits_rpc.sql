-- RPC for atomic expense + splits write.
--
-- The split-total constraint on public.expenses is DEFERRABLE INITIALLY
-- DEFERRED, so an expense plus its splits must be inserted within a single
-- transaction or commit will fail. PostgREST emits one transaction per REST
-- call, which is why direct inserts from the client cannot satisfy the
-- constraint. This RPC bundles both writes (and supports upsert + splits
-- replace) so clients have a transactional entry point.
