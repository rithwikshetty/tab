...
3. You have **registered the app in App Store Connect** at appstoreconnect.apple.com, under My Apps. You pick a name (display only — can change later), a unique bundle identifier that matches the Xcode project (for Tab this is `com.rithwikshetty.tab`), and an SKU. The SKU is purely an internal label for your own records — Apple never shows it to anyone — and any short string is fine.
